...
type AdapterFactory = (
  config: Pick<AppConfig, 'change' | 'storage'>,
) => SoftwareChangeCapability;
  ...
  [
    ...
    (config) =>
      new CodexSoftwareChangeCapability({
        command: config.change.adapters.codexCli.command,
        readinessTimeoutMs: config.storage.dependencyTimeoutMs,
        ...(config.change.adapters.codexCli.model === undefined
          ? {}
          : { model: config.change.adapters.codexCli.model }),
      }),
  ],
...
export function createSoftwareChangeCapabilityRegistry(
  ...
  const selectedId = canonicalSoftwareChangeAdapterId(config.change.adapterId);
