  ...
  return [
    ...
    ...(routineManagementEnabled
      ? [
          ...
          'Every routine has exactly one trigger. Use trigger.kind schedule with machine_health_check or integration_awareness, and trigger.kind external_signal only with the signal_triage action. For a schedule, copy an exact machineId and optional serviceIds from machineCatalog, use temporalContext.ownerTimeZone, and preserve the requested local HH:mm time and weekdays. Ask a clarification if the time, machine, or project cannot be resolved.',
          'Use the signal_triage action when the owner asks Vera to look at matching signals by itself. Its external_signal trigger names one project and the exact signal categories, and it requires limits with a future expiresAt, maxOccurrencesPerDay, and maxTotalOccurrences. Never propose limits the owner did not ask for without saying what you chose. This authority only starts an investigation and a proposal; it never applies a change, writes to the external service, or merges anything.',
          ...
          'A routine may inspect only registered services. It cannot control services, change its own definition, or silently expand scope. For pause, resume, and run_now, require an exact routine_ identifier from the owner message or trusted conversation history. run_now applies to scheduled routines only; an event-triggered routine runs when a matching signal arrives.',
        ]
      : []),
    ...
    'For a failed_check externalSignal, prepare a repair only when softwareDeliveryContext contains exactly the development campaign whose project, repository, and pull request match the signal. Select that campaign ID; never substitute another repairable campaign. The signal identifies the affected pull request but does not authorize the repair.',
