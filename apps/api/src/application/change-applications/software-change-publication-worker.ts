...
import type { SoftwareChangePublicationLifecycle } from './software-change-publication-lifecycle.ts';

export type SoftwareChangePublicationWorker = {
  start(): void;
  stop(): Promise<void>;
  wake(): void;
...
export function createSoftwareChangePublicationWorker(options: {
  workerId?: string;
  ...
  clock?: () => Date;
  createToken?: () => string;
}): SoftwareChangePublicationWorker {
  const workerId = options.workerId ?? `publication_worker_${randomUUID()}`;
    ...
    const runners = Array.from(
      ...
      async () => {
        while (next < candidates.length) {
          ...
          if (!claimed) continue;
          try {
            await options.lifecycle.progress(
            ...
            observer.warning(error, {
              operation: 'software_change_publication_worker_progress',
            ...
            await options.leases
              .release(candidate.project.id, token)
              .catch((error: unknown) => {
                observer.warning(error, {
                  operation: 'software_change_publication_worker_lease_release',
                  publicationId: candidate.id,
                  projectId: candidate.project.id,
                  workerId,
                });
  ...
  async function workLoop() {
    while (running) {
      try {
        ...
        observer.warning(error, {
          operation: 'software_change_publication_worker_poll',
          workerId,
        });
      }
      await waitForWork();
  ...
  return {
    ...
    async checkReadiness() {
      if (!running)
        throw new Error(
          'The software-change publication worker is not running.',
        );
