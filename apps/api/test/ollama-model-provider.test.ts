...
import { OllamaModelProvider } from '../src/model/ollama-model-provider.ts';
...
void describe('Ollama model adapter', () => {
  void it('requests structured output and normalizes provider metadata', async () => {
    ...
    const generation = await provider.generateProposal({ message: 'hello' });
  ...
  void it('classifies malformed provider JSON', async () => {
    ...
    await assert.rejects(
      provider.generateProposal({ message: 'hello' }),
      (error: unknown) =>
