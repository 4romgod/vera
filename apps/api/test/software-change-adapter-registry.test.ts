...
} from '../src/capabilities/software-change-adapter-registry.ts';
...
function config(adapterId: string): Pick<AppConfig, 'change' | 'storage'> {
  return {
    change: {
      adapterId,
      adapters: { codexCli: { command: 'codex', model: 'gpt-test' } },
    },
    storage: {
      mode: 'memory',
